...
## 2. Dynamic Progress Logic

| Test ID | File(s) Under Test | Target Widget / Logic | Scenario (It should...) | Setup & Mocks (Given) | Action (When) | Assertions (Then) |
|---|---|---|---|---|---|---|
| W-DL-TIL-05 | `download_task_tile.dart` | `_buildStatsSpans` | Format Playlist Item Counters | `totalItems=10`, `completedItems=3` | Check Stats Output | Displays "3/10" in the text block |
| W-DL-TIL-06 | `download_task_tile.dart` | `_computeEta` | Estimate Global ETA for Playlists | Playlist task running for 60 seconds, 50% complete | Evaluate `_computeEta` | Returns ~"1m 00s" |

## 3. User Interactions

| Test ID | File(s) Under Test | Target Widget / Logic | Scenario (It should...) | Setup & Mocks (Given) | Action (When) | Assertions (Then) |
|---|---|---|---|---|---|---|
| W-DL-TIL-07 | `download_task_tile.dart` | `DownloadTaskTile` | Trigger Cancel Confirmation | Task is `running` | Tap Cancel (X) icon | Overlay `_showCancelConfirm` appears |
| W-DL-TIL-08 | `download_task_tile.dart` | `_buildCancelConfirmation` | Cancel task and dismiss overlay | Overlay active | Tap "Yes, Cancel" | Calls `cancelDownload(id)`, overlay disappears |
| W-DL-TIL-09 | `download_task_tile.dart` | `DownloadTaskTile` | Remove completed task from UI | Task is `completed` | Tap Trash icon | Calls `removeTask(id)` on provider |
